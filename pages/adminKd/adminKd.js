...
Page({
        ...
        wx.showModal({
            ...
            success:(res)=>{
                if(res.confirm){
                    ...
                    wx.cloud.database().collection('shopKuaiDi').doc(temp._id).remove({
                        success:()=>{
                            wx.hideLoading()
    ...
    bottonRows(e){
       ...
       if(this.data.kdCode.length==0 || this.data.kdName.length==0)
       {
           ...
           wx.cloud.database().collection('shopKuaiDi').doc(id).update({
               data:{
                   kuaidiNum:this.data.kdCode,
                   kuaidiName:this.data.kdName,
                   status:'待收货'
               },success:(res)=>{
                   wx.hideLoading()
                    wx.showToast({
